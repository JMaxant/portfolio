...
// WCAG 1.4.4 (Resize Text, AA): content must not require horizontal scrolling when text is
// resized up to 200%. Only the narrowest widths are worth the extra run — wider ones have
// enough slack that 200% text rarely closes the gap, and this suite already covers them at
// the default size above.
const RESIZE_WIDTHS = [320, 576];

async function checkOverflow(page, path, width, { fontScale = 1 } = {}) {
  await page.setViewportSize({ width, height: 900 });
  await page.goto(path);

  if (fontScale !== 1) {
    await page.addStyleTag({ content: `html { font-size: ${fontScale * 100}% }` });
  }
  ...
  test.describe(`${width}px`, () => {
    for (const [name, path] of PAGES) {
      test(`${name} fits the viewport`, async ({ page }) => {
        await checkOverflow(page, path, width);
      });
    }
  });
}
...
for (const width of RESIZE_WIDTHS) {
  test.describe(`${width}px at 200% text`, () => {
    for (const [name, path] of PAGES) {
      test(`${name} fits the viewport with text resized to 200%`, async ({ page }) => {
        await checkOverflow(page, path, width, { fontScale: 2 });
      });
    }
  });
}
